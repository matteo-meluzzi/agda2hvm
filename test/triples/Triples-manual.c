...
#define _MAIN_ (0)
#define _TRIPLES_ (2)
#define _SUMALL_GO_ (5)
#define _CARTESIAN_ (8)
#define _FILTER_ (7)
#define _RANGE_ (13)
#define _ALLTRIPLES_ (9)
#define _SUMALL_ (1)
#define _GO_ (6)
          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _BIND_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 4u)) {
                ...
                link(mem, app_5 + 0, dp0_1);
                link(mem, app_5 + 1, ask_arg(mem, ask_arg(mem, term, 0), 0));
                u64 cal_6 = alloc(mem, 2);
                link(mem, cal_6 + 0, ask_arg(mem, ask_arg(mem, term, 0), 1));
                link(mem, cal_6 + 1, dp1_2);
                u64 cal_7 = alloc(mem, 2);
                link(mem, cal_7 + 0, App(app_5));
                link(mem, cal_7 + 1, Cal(2, 12, cal_6));
                u64 done = Cal(2, 14, cal_7);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 2);
            ...
            case _APPEND_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 4u)) {
                ...
                link(mem, cal_0 + 0, ask_arg(mem, ask_arg(mem, term, 0), 1));
                link(mem, cal_0 + 1, ask_arg(mem, term, 1));
                u64 ctr_1 = alloc(mem, 2);
                link(mem, ctr_1 + 0, ask_arg(mem, ask_arg(mem, term, 0), 0));
                link(mem, ctr_1 + 1, Cal(2, 14, cal_0));
                u64 done = Ctr(2, 4, ctr_1);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 2);
                clear(mem, get_loc(ask_arg(mem, term, 0), 0), 2);
                init = 1;
            ...
            case _TRIPLES_: {
              if (1) {
                ...
                link(mem, cal_3 + 0, Lam(lam_0));
                link(mem, cal_3 + 1, Cal(1, 9, cal_2));
                u64 done = Cal(2, 7, cal_3);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
            ...
            case _SUMALL_GO_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 11u)) {
                inc_cost(mem);
                u64 ret_4;
                if (get_tag(ask_arg(mem, ask_arg(mem, term, 0), 0)) == U32 && get_tag(ask_arg(mem, ask_arg(mem, term, 0), 1)) == U32) {
                  ret_4 = U_32(get_val(ask_arg(mem, ask_arg(mem, term, 0), 0)) + get_val(ask_arg(mem, ask_arg(mem, term, 0), 1)));
                  inc_cost(mem);
                } else {
                  u64 op2_5 = alloc(mem, 2);
                  link(mem, op2_5 + 0, ask_arg(mem, ask_arg(mem, term, 0), 0));
                  link(mem, op2_5 + 1, ask_arg(mem, ask_arg(mem, term, 0), 1));
                  ret_4 = Op2(ADD, op2_5);
                }
                u64 ret_2;
                if (get_tag(ret_4) == U32 && get_tag(ask_arg(mem, ask_arg(mem, term, 0), 2)) == U32) {
                  ret_2 = U_32(get_val(ret_4) + get_val(ask_arg(mem, ask_arg(mem, term, 0), 2)));
                  inc_cost(mem);
                } else {
                  u64 op2_3 = alloc(mem, 2);
                  link(mem, op2_3 + 0, ret_4);
                  link(mem, op2_3 + 1, ask_arg(mem, ask_arg(mem, term, 0), 2));
                  ret_2 = Op2(ADD, op2_3);
                }
                u64 cal_6 = alloc(mem, 1);
                link(mem, cal_6 + 0, ask_arg(mem, term, 1));
                u64 ret_0;
                if (get_tag(ret_2) == U32 && get_tag(Cal(1, 1, cal_6)) == U32) {
                  ret_0 = U_32(get_val(ret_2) + get_val(Cal(1, 1, cal_6)));
                  inc_cost(mem);
                } else {
                  u64 op2_1 = alloc(mem, 2);
                  link(mem, op2_1 + 0, ret_2);
                  link(mem, op2_1 + 1, Cal(1, 1, cal_6));
                  ret_0 = Op2(ADD, op2_1);
                }
                u64 done = ret_0;
                link(mem, host, done);
            ...
            case _CARTESIAN_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 11u)) {
                ...
                if (get_tag(dp0_11) == U32 && get_tag(dp1_12) == U32) {
                  ret_19 = U_32(get_val(dp0_11) * get_val(dp1_12));
                  inc_cost(mem);
                } else {
                  u64 op2_20 = alloc(mem, 2);
                  link(mem, op2_20 + 0, dp0_11);
                  link(mem, op2_20 + 1, dp1_12);
                  ret_19 = Op2(MUL, op2_20);
                }
                u64 ret_21;
                if (get_tag(dp0_6) == U32 && get_tag(dp1_7) == U32) {
                  ret_21 = U_32(get_val(dp0_6) * get_val(dp1_7));
                  inc_cost(mem);
                } else {
                  u64 op2_22 = alloc(mem, 2);
                  link(mem, op2_22 + 0, dp0_6);
                  link(mem, op2_22 + 1, dp1_7);
                  ret_21 = Op2(MUL, op2_22);
                }
                u64 ret_17;
                if (get_tag(ret_19) == U32 && get_tag(ret_21) == U32) {
                  ret_17 = U_32(get_val(ret_19) + get_val(ret_21));
                  inc_cost(mem);
                } else {
                  u64 op2_18 = alloc(mem, 2);
                  link(mem, op2_18 + 0, ret_19);
                  link(mem, op2_18 + 1, ret_21);
                  ret_17 = Op2(ADD, op2_18);
                }
                u64 ret_23;
                if (get_tag(dp0_1) == U32 && get_tag(dp1_2) == U32) {
                  ret_23 = U_32(get_val(dp0_1) * get_val(dp1_2));
                  inc_cost(mem);
                } else {
                  u64 op2_24 = alloc(mem, 2);
                  link(mem, op2_24 + 0, dp0_1);
                  link(mem, op2_24 + 1, dp1_2);
                  ret_23 = Op2(MUL, op2_24);
                }
                u64 ret_15;
                if (get_tag(ret_17) == U32 && get_tag(ret_23) == U32) {
                  ret_15 = U_32(get_val(ret_17) == get_val(ret_23) ? 1 : 0);
                  inc_cost(mem);
                } else {
                  u64 op2_16 = alloc(mem, 2);
                  link(mem, op2_16 + 0, ret_17);
                  link(mem, op2_16 + 1, ret_23);
                  ret_15 = Op2(EQL, op2_16);
                }
                u64 done = ret_15;
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
            ...
            case _FILTER_: {
              ...
              if ((get_tag(ask_arg(mem, term, 1)) == CTR && get_ext(ask_arg(mem, term, 1)) == 4u)) {
                ...
                link(mem, app_20 + 0, dp1_12);
                link(mem, app_20 + 1, dp0_6);
                u64 cal_21 = alloc(mem, 2);
                link(mem, cal_21 + 0, dp0_16);
                link(mem, cal_21 + 1, dp0_1);
                u64 ctr_22 = alloc(mem, 2);
                link(mem, ctr_22 + 0, dp1_7);
                link(mem, ctr_22 + 1, Cal(2, 7, cal_21));
                u64 cal_23 = alloc(mem, 2);
                link(mem, cal_23 + 0, dp1_17);
                link(mem, cal_23 + 1, dp1_2);
                u64 cal_24 = alloc(mem, 3);
                link(mem, cal_24 + 0, App(app_20));
                link(mem, cal_24 + 1, Ctr(2, 4, ctr_22));
                link(mem, cal_24 + 2, Cal(2, 7, cal_23));
                u64 done = Cal(3, 10, cal_24);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 2);
            ...
            case _RANGE_: {
              if (1) {
                ...
                u64 ret_5;
                if (get_tag(ret_7) == U32 && get_tag(dp0_1) == U32) {
                  ret_5 = U_32(get_val(ret_7) - get_val(dp0_1));
                  inc_cost(mem);
                } else {
                  u64 op2_6 = alloc(mem, 2);
                  link(mem, op2_6 + 0, ret_7);
                  link(mem, op2_6 + 1, dp0_1);
                  ret_5 = Op2(SUB, op2_6);
                }
                u64 cal_9 = alloc(mem, 2);
                link(mem, cal_9 + 0, ret_5);
                link(mem, cal_9 + 1, dp1_2);
                u64 done = Cal(2, 6, cal_9);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 2);
                init = 1;
                continue;
            ...
            case _ALLTRIPLES_: {
              if (1) {
                inc_cost(mem);
                u64 cal_0 = alloc(mem, 2);
                link(mem, cal_0 + 0, U_32(1));
                link(mem, cal_0 + 1, ask_arg(mem, term, 0));
                u64 lam_1 = alloc(mem, 2);
                u64 cpy_2 = Var(lam_1);
                u64 dp0_3;
                u64 dp1_4;
                if (get_tag(cpy_2) == U32) {
                  inc_cost(mem);
                  dp0_3 = cpy_2;
                  dp1_4 = cpy_2;
                } else {
                ...
                u64 cal_7 = alloc(mem, 2);
                link(mem, cal_7 + 0, U_32(1));
                link(mem, cal_7 + 1, dp0_3);
                u64 lam_8 = alloc(mem, 2);
                u64 cpy_9 = Var(lam_8);
                u64 dp0_10;
                u64 dp1_11;
                if (get_tag(cpy_9) == U32) {
                  inc_cost(mem);
                  dp0_10 = cpy_9;
                  dp1_11 = cpy_9;
                } else {
                  u64 dup_12 = alloc(mem, 3);
                  u64 col_13 = gen_dupk(mem);
                  link(mem, dup_12 + 2, cpy_9);
                  dp0_10 = Dp0(col_13, dup_12);
                  dp1_11 = Dp1(col_13, dup_12);
                }
                u64 cal_14 = alloc(mem, 2);
                link(mem, cal_14 + 0, U_32(1));
                link(mem, cal_14 + 1, dp0_10);
                u64 lam_15 = alloc(mem, 2);
                u64 ctr_16 = alloc(mem, 3);
                link(mem, ctr_16 + 0, Var(lam_15));
                link(mem, ctr_16 + 1, dp1_11);
                link(mem, ctr_16 + 2, dp1_4);
                u64 ctr_17 = alloc(mem, 0);
                u64 ctr_18 = alloc(mem, 2);
                link(mem, ctr_18 + 0, Ctr(3, 11, ctr_16));
                link(mem, ctr_18 + 1, Ctr(0, 3, ctr_17));
                link(mem, lam_15 + 1, Ctr(2, 4, ctr_18));
                u64 cal_19 = alloc(mem, 2);
                link(mem, cal_19 + 0, Cal(2, 13, cal_14));
                link(mem, cal_19 + 1, Lam(lam_15));
                link(mem, lam_8 + 1, Cal(2, 12, cal_19));
                u64 cal_20 = alloc(mem, 2);
                link(mem, cal_20 + 0, Cal(2, 13, cal_7));
                link(mem, cal_20 + 1, Lam(lam_8));
                link(mem, lam_1 + 1, Cal(2, 12, cal_20));
                u64 cal_21 = alloc(mem, 2);
                link(mem, cal_21 + 0, Cal(2, 13, cal_0));
                link(mem, cal_21 + 1, Lam(lam_1));
                u64 done = Cal(2, 12, cal_21);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
            ...
            case _SUMALL_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 4u)) {
                inc_cost(mem);
                u64 cal_0 = alloc(mem, 2);
                link(mem, cal_0 + 0, ask_arg(mem, ask_arg(mem, term, 0), 0));
                link(mem, cal_0 + 1, ask_arg(mem, ask_arg(mem, term, 0), 1));
                u64 done = Cal(2, 5, cal_0);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                clear(mem, get_loc(ask_arg(mem, term, 0), 0), 2);
                init = 1;
            ...
            case _GO_: {
              ...
              if (1) {
                ...
                u64 ret_7;
                if (get_tag(U_32(1)) == U32 && get_tag(dp1_2) == U32) {
                  ret_7 = U_32(get_val(U_32(1)) + get_val(dp1_2));
                  inc_cost(mem);
                } else {
                  u64 op2_8 = alloc(mem, 2);
                  link(mem, op2_8 + 0, U_32(1));
                  link(mem, op2_8 + 1, dp1_2);
                  ret_7 = Op2(ADD, op2_8);
                }
                u64 cal_9 = alloc(mem, 2);
                link(mem, cal_9 + 0, ret_5);
                link(mem, cal_9 + 1, ret_7);
                u64 ctr_10 = alloc(mem, 2);
                link(mem, ctr_10 + 0, dp0_1);
                link(mem, ctr_10 + 1, Cal(2, 6, cal_9));
                u64 done = Ctr(2, 4, ctr_10);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 2);
                init = 1;
                continue;
...
int main(int argc, char* argv[]) {
  ...
  // Id-to-Name map
  const u64 id_to_name_size = 15;
  char* id_to_name_data[id_to_name_size];
  id_to_name_data[6] = "Go";
  id_to_name_data[7] = "Filter";
  id_to_name_data[14] = "Append";
  id_to_name_data[2] = "Triples";
  id_to_name_data[1] = "Sumall";
  id_to_name_data[11] = "Triple";
  id_to_name_data[8] = "Cartesian";
  id_to_name_data[13] = "Range";
  id_to_name_data[10] = "If";
  id_to_name_data[4] = "Cons";
  id_to_name_data[5] = "Sumall.Go";
  id_to_name_data[9] = "Alltriples";
  id_to_name_data[3] = "Nil";
  id_to_name_data[0] = "Main";
  id_to_name_data[12] = "Bind";
;
